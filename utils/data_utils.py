...
def download_ohlcv_basic(ticker, start=None, end=None, interval="1d", period=None):
    ...
    if ticker.endswith(".TW") and not cached_df.empty:
        print(f"Taiwan Stock detected. Using local database cache for {ticker} ({interval}) to bypass yfinance timezone bugs.")
        need_download = False
...
def download_ohlcv_with_fallback(ticker, start, end, interval, fallback_periods=("730d", "365d", "180d", "90d", "60d")):
    print(f"Checking local database cache for {ticker}, interval={interval}, start={start}, end={end} ...")
    try:
        df = download_ohlcv_basic(ticker=ticker, start=start, end=end, interval=interval)
        if not df.empty:
            print(f"Loaded from local database: rows={len(df)}, {df.index.min()} -> {df.index.max()}")
            return df
    except Exception as e:
...
def download_and_build_mtf(cfg, progress_callback=None):
    ...
    if progress_callback:
        progress_callback(f"Building MTF features... (W1: {w1_bars} bars ✓)")
